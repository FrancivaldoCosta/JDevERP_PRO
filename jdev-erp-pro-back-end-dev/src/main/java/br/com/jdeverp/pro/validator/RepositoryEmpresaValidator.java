	...
	public void afterSingletonsInstantiated() {
		...
		for (Class<?> interfaceRepository: getRepositories()) {
			...
			for (Method method: interfaceRepository.getMethods()) {
				...
				/* Ignora apenas esse método que contém @IgnoreEmpresaId */
				...
				/* Verificar se tem @Query escrito no método */
				boolean queryPresent = method.isAnnotationPresent(Query.class);
				
				if (!queryPresent) {
					throw new IllegalStateException("O método: " + method + " da interface: " + interfaceRepository + " deve possuir Query escrita.");
				}
				...
				/* Continua se tiver empresaId e está correto */
